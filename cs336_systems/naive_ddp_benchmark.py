...
def flat_ddp(model, data, optimizer, num_trials, num_warmup_trials, step_times, communication_times):
    for _ in range(num_warmup_trials):
        ...
        flatten_params = torch._utils.flatten_dense_tensors(model.parameters())
        dist.all_reduce(flatten_params, op=dist.ReduceOp.SUM, async_op=False)

        unflatten_params = torch._utils.unflatten_dense_tensors(flatten_params, model.parameters())
...
def benchmark_driver(rank, world_size, data, num_layers, batch_size,
    ...
    # prepare empty “buckets” to receive each rank’s tensor
...
if __name__ == "__main__":
    ...
    # type of ddp
    parser.add_argument("--ddp_type", type=str, default="naive", choices=["naive", "flat_ddp"])
